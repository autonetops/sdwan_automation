...
def test_waits_until_success(credentials):
    ...
    responses.add(
        responses.GET,
        STATUS_URL,
        json={"summary": {"status": "in_progress"}, "data": []},
        status=200,
    )
    responses.add(
        responses.GET,
        ...
        json={
            ...
            "data": [{"host-name": "Site1-Edge1", "status": "Success"}],
        },
        status=200,
    )
...
def test_failure_raises_and_names_the_guilty_device(credentials):
    ...
    responses.add(
        ...
        json={
            ...
            "data": [
                {
                    "host-name": "Site2-Edge1",
                    "status": "Failure",
                    "currentActivity": "Template rollback",
                }
            ],
        },
        status=200,
...
def test_the_pipeline_can_ask_it_not_to_raise(credentials):
    ...
    responses.add(
        responses.GET,
        STATUS_URL,
        json={"summary": {"status": "failure"}, "data": []},
        status=200,
    )
    client = SDWANClient(credentials, min_interval=0).login()
    result = wait_for_task(
        client, TASK_ID, interval=0, timeout=30, raise_on_failure=False
    )
