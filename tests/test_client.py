...
LOGIN_HTML = (
    "<html><head><title>Cisco Catalyst SD-WAN</title></head><body>login</body></html>"
)
...
def test_bad_credentials_return_200_with_html(credentials):
    ...
    responses.add(
        responses.POST, f"{BASE_URL}/j_security_check", body=LOGIN_HTML, status=200
    )
...
def test_get_unwraps_the_data_envelope(credentials, device_payload):
    ...
    responses.add(
        ...
        json={"header": {}, "data": device_payload},
        status=200,
    )
...
def test_the_dataservice_prefix_is_optional(credentials):
    ...
    responses.add(
        responses.GET,
        f"{BASE_URL}/dataservice/device/counters",
        json={"data": [{"ok": True}]},
        status=200,
    )
    client = SDWANClient(credentials, min_interval=0).login()
...
def test_an_expired_xsrf_token_gets_its_own_message(credentials):
    ...
    responses.add(
        responses.POST,
        f"{BASE_URL}/dataservice/device/action/deploy",
        body="Invalid XSRF token",
        status=403,
    )
    client = SDWANClient(credentials, min_interval=0).login()
